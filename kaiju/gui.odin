...
import "../deps/odin-imgui/imgui_impl_sdl2"
import "../deps/odin-imgui/imgui_impl_dx12"
...
g_pd3dDevice: ^d3d12.IDevice
g_pSwapChain: ^dxgi.ISwapChain
g_mainRenderTargetView: ^d3d12.RENDER_TARGET_VIEW_DESC
...
main :: proc() {
	...
	imgui_impl_dx12.Init(g_pd3dDevice)
	defer imgui_impl_dx12.Shutdown()
	...
	for running {
		...
		imgui_impl_dx12.RenderDrawData(im.GetDrawData())
...
create_device_d3d :: proc(hwnd: windows.HWND) -> bool {
	...
	create_device_flags: d3d12.CREATE_DEVICE_FLAGS
	feature_level: d3d12.FEATURE_LEVEL
	feature_levels: []d3d12.FEATURE_LEVEL = { ._12_0, ._11_0 }
	if d3d12.CreateDeviceAndSwapChain(
		nil,
		.HARDWARE,
		nil,
		create_device_flags,
...
create_render_target :: proc() {
	backbuffer: ^d3d12.ITexture2D
	g_pSwapChain->GetBuffer(0, d3d12.ITexture2D_UUID, cast(^rawptr)&backbuffer)
